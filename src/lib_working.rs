...
const UNISWAP_ROUTER_ADDRESS: &str = "0x53544492C78D478294B5b1Fb8059a5D6C9aBBC67"; // Replace with actual Uniswap router address on Sepolia
const USDC_ADDRESS: &str = "0xf08a50178dfcde18524640ea6618a1f965821715"; // Replace with actual USDC token address on Sepolia

#[update]
pub async fn swap_eth_for_usdc(amount_in_eth: Nat, amount_out_min: Nat, recipient: String) -> String {
    let caller = validate_caller_not_anonymous();

    // Convert addresses to Ethereum Address type
    // let router_address = Address::from_str(UNISWAP_ROUTER_ADDRESS).expect("Invalid router address");
    // let router_address: alloy_primitives::Address = alloy_primitives::Address::from(router_address.0);
    ...
    // Define the Uniswap swap path: ETH -> USDC
    let path = vec![
        Address::from_str("0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE").unwrap(), // Representing ETH
        usdc_address,
    ];
    ...
    // Deadline: current timestamp + buffer time (e.g., 300 seconds)
    let deadline = ic_cdk::api::time() / 1_000_000_000 + 300;

    // Prepare the data for the function call
    // let tx_data = encode_swap_eth_for_tokens_function_call(amount_in_eth, amount_out_min, path, recipient_address, deadline);
    let tx_data = encode_swap_eth_for_tokens_function_call(amount_in_eth.clone(), amount_out_min, path, recipient_address, deadline);

    // let router_address = Address::from_str(UNISWAP_ROUTER_ADDRESS).expect("Invalid router address").0;
    // let router_address = alloy_primitives::Address::from(router_address); // Convert to alloy_primitives::Address

    // let router_address = Address::from_str(UNISWAP_ROUTER_ADDRESS).expect("Invalid router address");
    // let router_address = alloy_primitives::Address::from(router_address.as_slice());
    // let router_address = alloy_primitives::Address::from(router_address.0);
    let router_address = alloy_primitives::Address::from_str(UNISWAP_ROUTER_ADDRESS).expect("Invalid router address");
    ...
    let transaction = TxEip1559 {
        ...
        to: TxKind::Call(router_address), // Now correctly typed
        value: nat_to_u256(amount_in_eth),
        access_list: Default::default(),
        input: tx_data.into(), // Convert Vec<u8> to Bytes
    };
    ...
    let (result,) = EVM_RPC
        .eth_send_raw_transaction(single_rpc_service, None, raw_transaction_hex.clone(), 2_000_000_000_u128)
        .await
        .expect("Failed to send transaction");

    ic_cdk::println!("Transaction sent with hash: {}", raw_transaction_hash);
    raw_transaction_hash.to_string()
}

fn encode_swap_eth_for_tokens_function_call(
    amount_in_eth: Nat,
    amount_out_min: Nat,
    path: Vec<Address>,
    recipient: Address,
    ...
    // Encode function call for Uniswap's swapExactETHForTokens
    // ABI encoding details will depend on your specific setup and use case
    unimplemented!()
}
