...
pub async fn transaction_count(owner: Option<Principal>, block: Option<BlockTag>) -> Nat {
    ...
    let args = GetTransactionCountArgs {
        address,
        block: block.unwrap_or(BlockTag::Latest),
    };

    // Improved error handling for the RPC call
    let rpc_result = EVM_RPC
    ...
    match rpc_result {
        Ok((result,)) => {
            ...
            ic_cdk::println!("Transaction count result: {:?}", result);
            ...
            match result {
                MultiGetTransactionCountResult::Consistent(consistent_result) => match consistent_result {
                    ...
                    GetTransactionCountResult::Err(error) => {
                        ic_cdk::trap(&format!(
                            "Error in getting transaction count for {:?}, error: {:?}",
                            args, error
                        ))
                    }
                },
                MultiGetTransactionCountResult::Inconsistent(inconsistent_results) => {
                    ic_cdk::trap(&format!(
                        "Inconsistent results when retrieving transaction count for {:?}. \
                         Received results: {:?}",
                        args, inconsistent_results
                    ))
                }
            }
        }
        // Trap with detailed error if RPC call fails
        Err(e) => {
            ic_cdk::trap(&format!(
                "Failed to get transaction count for {:?}, RPC error: {:?}",
                args, e
            ))
        }
    }
...
pub async fn send_usdc(to: String, amount: Nat) -> String {
    ...
    // Prepare the transfer data according to the ERC-20 transfer method
...
fn estimate_transaction_fees() -> (u128, u128, u128) {
    /// Updated gas limit to account for additional transaction requirements.
    const GAS_LIMIT: u128 = 100_000;

    /// Crude estimates of max_fee_per_gas and max_priority_fee_per_gas.
    /// In a production environment, these should be fetched dynamically.
    ...
    fn from(value: &EcdsaKeyName) -> Self {
        EcdsaKeyId {
            ...
            name: match value {
                ...
                EcdsaKeyName::TestKeyLocalDevelopment => "key_1",
